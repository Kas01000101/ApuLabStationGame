...
const LEVEL_EVENTS: Record<number, Set<string>> = {
  ...
  7: new Set(['sample_reached','sample_analyze_requested','instrument_modal_opened','instrument_selected','sample_analyzed','instrument_changed','relevant_instrument_selected','final_point_reached']),
};
...
async function readProof(token: unknown): Promise<Proof> {
  ...
  if (!proof || proof.exp < Date.now() || !UUID_PATTERN.test(proof.participant_id) || !proof.study_id || proof.study_condition !== 'game') throw new PublicError('session_proof_invalid', 403);
  return proof;
}
...
async function authFailureCount(client: ReturnType<typeof admin>, hash: string): Promise<{ count:number; cooldownUntil:string|null }> {
  ...
  const { data, error } = await client.from('apulab_auth_attempts')
    .select('attempted_at,cooldown_until').eq('participant_code_hash', hash).eq('success', false).gte('attempted_at', since)
  ...
  try {
    ...
    if (url.pathname.endsWith('/session/complete')) {
      ...
      const { data:session, error } = await client.from('apulab_sessions')
        .select('session_id,participant_id,study_id,study_condition,session_mode,sync_token_hash,status').eq('session_id', sessionId).maybeSingle();
      ...
      const { data:events, error:eventError } = await client.from('apulab_events')
        .select('event_type,level_number,participant_id,study_id,study_condition').eq('session_id', sessionId)
        .in('event_type', ['level_completed','session_completed']);
      if (eventError) throw new Error('completion_lookup_failed');
      const rows = events ?? [];
      const belongsToSession = (event:any) => event.participant_id === session.participant_id
        && event.study_id === session.study_id && event.study_condition === session.study_condition;
      const hasN7Complete = rows.some((event:any) => belongsToSession(event) && event.event_type === 'level_completed' && event.level_number === 7);
      const hasSessionComplete = rows.some((event:any) => belongsToSession(event) && event.event_type === 'session_completed');
      if (!hasN7Complete || !hasSessionComplete) throw new PublicError('session_not_complete', 409);
      const { error:updateError } = await client.from('apulab_sessions')
        ...
        .eq('session_id', sessionId).neq('status', 'completed');
    ...
    if (url.pathname.endsWith('/events')) {
      ...
      for (const row of rows) {
        ...
        if (existing && (existing.session_id !== row.session_id || existing.event_seq !== row.event_seq || existing.event_type !== row.event_type)) throw new PublicError('event_id_conflict', 409);
      }
