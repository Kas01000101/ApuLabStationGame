...
DROP VIEW IF EXISTS public.v_level6_science_metrics;
DROP VIEW IF EXISTS public.v_level7_instrument_metrics;
DROP VIEW IF EXISTS public.v_session_quality;

CREATE OR REPLACE VIEW public.v_official_study_events WITH (security_invoker=true) AS
SELECT * FROM public.apulab_events
WHERE study_id='APULAB-STUDY-2026' AND environment='study';

CREATE OR REPLACE VIEW public.v_qa_events WITH (security_invoker=true) AS
SELECT * FROM public.apulab_events
WHERE study_id='APULAB-QA-2026';

CREATE VIEW public.v_level_outcomes WITH (security_invoker=true) AS
...
CREATE VIEW public.v_level5_loop_metrics WITH (security_invoker=true) AS
WITH e AS (
...
CREATE VIEW public.v_level6_science_metrics WITH (security_invoker=true) AS
WITH e AS (
  SELECT * FROM public.v_official_study_events WHERE level_number=6
), seq AS (
  SELECT session_id,participant_id,study_id,study_condition,build_version,git_commit_sha,
...
CREATE VIEW public.v_level7_instrument_metrics WITH (security_invoker=true) AS
WITH e AS (
  SELECT * FROM public.v_official_study_events WHERE level_number=7
), ordered AS (
  SELECT *,
    CASE
...
CREATE VIEW public.v_session_quality WITH (security_invoker=true) AS
WITH base AS (
  ...
  LEFT JOIN public.apulab_events e USING(session_id)
