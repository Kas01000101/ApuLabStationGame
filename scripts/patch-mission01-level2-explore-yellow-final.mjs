...
(() => {
  ...
  const clearYellow = () => {
    if (!yellowApplied) return;
    panel.style.removeProperty('background');panel.style.removeProperty('color');panel.style.removeProperty('border-color');panel.style.removeProperty('box-shadow');
    targets.forEach((node) => node.style.removeProperty('color'));yellowApplied = false;
  };
  const sync = () => {
    const label = String(title.textContent || '').replace(/\\s+/g, ' ').trim();
    const shouldBeYellow = !panel.hidden && !panel.classList.contains('is-guide') && stepPattern.test(label);
    if (shouldBeYellow) setYellow(); else clearYellow();
  };
  sync();
  ...
  const cleanup=()=>{panelObserver.disconnect();titleObserver.disconnect();};
  window.addEventListener('pagehide',cleanup,{once:true});window.addEventListener('beforeunload',cleanup,{once:true});
  window.addEventListener('message',(event)=>{if(event.data&&event.data.type==='apulab-dispose')cleanup();});
})();
</script>`;

if (!html.includes('</body>')) throw new Error('mission01_level2_explore_yellow_final_invalid_html');
