...
async function buildLevel1() {
  const encoded = await concatTextFiles(resolve(MISSION_SRC, 'source/level1'), /^part\d+\.b64$/);
  return patchLevel1(Buffer.from(encoded, 'base64').toString('utf8'));
}

async function buildLevel2() {
  const encoded = await concatTextFiles(resolve(MISSION_SRC, 'source/level2'), /^part\d+\.b64$/);
  return patchLevel2(Buffer.from(encoded, 'base64').toString('utf8'));
}
...
async function verifyAndWrite(level, html) {
  ...
  if (actualHash !== expected.sha256 || actualBytes !== expected.bytes) {
    throw new Error(
      `mission01_integrity_failed:level${level}:sha=${actualHash}:bytes=${actualBytes}`,
    );
  }
