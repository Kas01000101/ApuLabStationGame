...
for (const level of [1, 2, 3, 4, 5, 6, 7]) {
  ...
  if (canonicalDerived) {
    ...
    if (!/(?:const\s+)?DESIGN_(?:W|WIDTH)\s*=\s*1672/.test(html)) fail('n5_design_width_missing', `l${level}`);
    if (!/(?:const\s+)?DESIGN_(?:H|HEIGHT)\s*=\s*941/.test(html)) fail('n5_design_height_missing', `l${level}`);
    if (!/stage\.style\.transform\s*=\s*`scale\(\$\{s\}\)`/.test(html) && !html.includes("stage.style.setProperty('--apulab-stage-scale'")) {
      fail('scale_application_missing', `l${level}`);
    }
