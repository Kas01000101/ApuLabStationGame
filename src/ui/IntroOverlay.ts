...
export class IntroOverlay {
  ...
  constructor(parent: HTMLElement, options: IntroOverlayOptions = {}) {
    ...
    this.root.innerHTML = `
      ...
      <div class="intro-dialogue" aria-live="polite">
        <div class="intro-speaker"></div>
        <div class="intro-dialogue-text"></div>
      </div>
      <div class="intro-sfx"></div>
      <div class="intro-beat"></div>
      <div class="intro-mission">
        <div class="intro-mission-small">MISIÓN 01</div>
        <div class="intro-mission-big">1 / 3 · MEDIR</div>
      </div>
      <div class="nickname-overlay">
        <div class="nickname-card">
          ...
          <button class="nickname-continue" type="button">CONTINUAR</button>
        </div>
      </div>
    `;
    ...
    this.require<HTMLButtonElement>('.intro-skip').addEventListener('click', () => options.onSkip?.());
  ...
  showSfx(text: string, strength = 1): void {
    this.sfx.textContent = text;
    this.sfx.style.opacity = String(Math.max(0, Math.min(1, strength)));
    this.sfx.style.transform = `translate(-50%,-50%) scale(${0.82 + 0.25 * strength}) rotate(-2deg)`;
  }

  hideSfx(): void {
    this.sfx.textContent = '';
    this.sfx.style.opacity = '0';
  }

  showLocation(text: string, station = false): void {
    this.location.textContent = text;
    this.location.classList.toggle('station', station);
    this.location.classList.add('show');
  }
  ...
  requestNickname(onSubmit: (nickname: string) => void): void {
    this.nicknameSubmit = onSubmit;
    this.nicknameError.textContent = '';
    this.nicknameInput.value = '';
    this.nicknamePanel.classList.add('show');
  ...
  private commitNickname(): void {
    ...
    if (!nickname) {
      this.nicknameError.textContent = 'Escribe un apodo para continuar.';
      this.nicknameInput.focus();
      return;
    }
    const submit = this.nicknameSubmit;
    if (!submit) return;
    this.closeNickname();
    submit(nickname);
  }
  ...
  private require<T extends Element>(selector: string): T {
    ...
    if (!element) throw new Error(`intro_overlay_missing:${selector}`);
    return element;
  }
}
