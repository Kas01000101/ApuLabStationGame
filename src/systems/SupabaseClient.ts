import type { RepositoryResult, AuthenticatedParticipant } from './research/ResearchRepository';
...
export class SupabaseClient {
  ...
  static authenticateParticipant(studyCode: string, credential: string): Promise<RepositoryResult<AuthenticatedParticipant>> {
    return this.post('/authenticate', { study_code: studyCode, credential });
  }
  ...
  static async post<T = void>(path: string, body: unknown): Promise<RepositoryResult<T>> {
    ...
    try {
      ...
      return { success: false, error: error instanceof DOMException && error.name === 'AbortError' ? 'request_timeout' : 'network_error' };
