...
export const COMMON_EVENT_TYPES = new Set<string>(COMMON_EVENTS);

export const LEVEL_EVENT_TYPES: Readonly<Record<number, ReadonlySet<string>>> = Object.freeze(
  Object.fromEntries(
    Object.entries(LEVEL_EVENTS).map(([level, events]) => [Number(level), new Set<string>(events)]),
  ),
);

/** Runtime-only aliases. They preserve the event that actually occurred. */
export const COMPATIBILITY_EVENT_ALIASES: Readonly<Record<string, ResearchEventType | null>> = {
  ...
  final_checkpoint_reached: 'final_point_reached',
