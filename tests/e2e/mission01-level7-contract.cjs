...
(async()=>{
  ...
  let s=await state();assert(s.atSample===false,'L7: adjacency incorrectly counted as sample cell');assert(s.sampleReached===false,'L7: adjacency incorrectly set sampleReached');assert(!await page.locator('.block-analyze-sample').evaluate(el=>el.classList.contains('is-ready')),'L7: analyze glows while AYNI is beside sample');assert(!await page.locator('#success-overlay').isVisible(),'L7: adjacent analyze completed mission');
  ...
  s=await state();assert(s.atSample===true,'L7: AYNI did not finish exactly on sample');assert(s.sampleReached===true,'L7: exact sample occupancy did not set sampleReached');assert(await page.locator('.block-analyze-sample').evaluate(el=>el.classList.contains('is-ready')),'L7: ANALIZAR MUESTRA not highlighted while AYNI is on sample');assert((await page.locator('#objective-tag').textContent()||'').includes('ANALIZA LA MUESTRA'),'L7: guide/objective did not advance to analysis');await shot('03-ayni-on-sample');
  ...
  await openLevel();const withoutSend=[...EXACT_SAMPLE_ROUTE,'analyzeSample',...TO_COMMUNICATION];await addCommands(withoutSend);await page.locator('#run-btn').click();await page.locator('#sensor-overlay.visible').waitFor({timeout:25000});await choose('materials');await continueAnalysis();await page.waitForTimeout(900);s=await state();assert(s.communicationPointReached===true,'L7: communication point not reached');assert(s.dataSent===false,'L7: dataSent became true without ENVIAR DATOS');assert(!await page.locator('#success-overlay').isVisible(),'L7: reaching checkpoint 2 completed mission without send');assert(await page.locator('.block-send').evaluate(el=>el.classList.contains('is-ready')),'L7: ENVIAR DATOS not highlighted at checkpoint 2');await shot('05-communication-waits-for-send');
