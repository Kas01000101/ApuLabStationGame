...
// Start A? -> exact sample (5,2): old adjacent route plus LEFT + FORWARD.
...
// From exact sample facing north -> communication (6,6).
...
(async()=>{
  ...
  // Input contract: each interaction inserts exactly one ANALIZAR MUESTRA.
  ...
  // Premature send before science: soft feedback, program preserved, no completion.
  await addCommands(['send']);await page.locator('#run-btn').click();await page.waitForFunction(()=>(document.getElementById('feedback')?.textContent||'').includes('Aún necesitas el dato'));
  assert(await page.locator('.program-block').count()===1,'L7: premature send erased program');assert(!await page.locator('#success-overlay').isVisible(),'L7: premature send completed mission');await clearProgram();

  // Exact-cell contract: adjacent is NOT enough and does not open selector.
  ...
  let s=await state();assert(s.sampleCheckpointReached===false,'L7: adjacent rover incorrectly triggered sample_reached');assert(!await page.locator('#sensor-overlay').isVisible(),'L7: adjacent analyze opened selector');assert(await page.locator('.program-block').count()===TO_ADJACENT.length+1,'L7: adjacent failure erased program');

  // Exact sample opens science flow and drives step 2/3 from real state.
  ...
  // All three instruments provide valid information; materials answers composition.
  ...
  // Materials-first is valid. Arrival at communication must NOT auto-send/complete.
  await openLevel();
  ...
  let events=await telemetry();let names=events.map(x=>x.event);assert(names.includes('communication_point_reached'),'L7: canonical communication event missing');assert(!names.includes('data_sent'),'L7: data_sent emitted automatically on arrival');assert(!names.includes('final_point_reached'),'L7: legacy final_point_reached emitted');

  // Relevant data but SEND away from communication is also a soft non-completing action.
  await openLevel();await addCommands([...TO_SAMPLE,'analyzeSample','send']);await page.locator('#run-btn').click();await page.locator('#sensor-overlay.visible').waitFor({timeout:30000});await choose('materials');await continueAnalysis();await page.waitForFunction(()=>(document.getElementById('feedback')?.textContent||'').includes('punto de comunicación'),null,{timeout:12000});
  ...
  // Final canonical path: exact sample -> materials -> communication -> explicit SEND -> complete.
  await openLevel();
